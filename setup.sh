...
#
# This is a living checklist and not a finished installer. It documents
# what a fresh clone needs right now and grows as more features are added.
# Update it whenever a new one-time step is added.

set -eu
cd "$(dirname "$0")"

# 1. uv itself is a prerequisite and not something this script installs for you.
...
echo
echo "Manual steps not handled by this script:"
echo
...
echo
echo "2. Create .env for your secrets and email addresses:  cp .env.example .env"
echo
echo "3. Put your CV in profile/ as markdown (profile/cv.md)."
echo
echo "Check everything works in the host:"
echo
...
echo "  uv run job-hunters discover --dry-run                   what the discover sources would queue (free)"
echo "  uv run job-hunters promote --review                     companies waiting to join the watchlist"
echo
echo "Check everything works in the containers:"
echo
echo "  docker compose up -d                                    start web and scheduler"
echo "  curl -s http://127.0.0.1:8000/health                    the web service answers {\"Status\":\"Ok\"}"
echo "  docker compose logs scheduler | grep Registered         four jobs on their schedules"
echo "  docker compose exec scheduler job-hunters show-config   the container sees config/ and .env"
