...
1. **The search:** edit `config/search_profile.yaml`, `config/system_config.yaml` and `config/companies_watchlist.yaml`.
...
3. **Your CV:** put it in `profile/cv.md` as a markdown file. Every markdown file in `profile/` is joined into the judge's prompt, so keep only what the judge should read.
