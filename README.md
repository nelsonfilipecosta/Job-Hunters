...
1. **Secrets:** `cp .env.example .env` and fill it in with the API key, a random `ACTION_TOKEN_SECRET` (the file says how to generate one), the email address the digest goes to and the SMTP credentials it is sent with.
2. **Your CV:** put it in `profile/cv.md` as a markdown file. Every markdown file in `profile/` is joined into the judge's prompt, so keep only what the judge should read.
3. **The search:** edit `config/search_profile.yaml`, `config/system_config.yaml` and `config/companies_watchlist.yaml`.
