...
def test_a_token_survives_a_round_trip() -> None:
    ...
    signed = verify(SECRET, sign(SECRET, Action.APPLIED, 42, now=NOW), now=NOW)
...
def test_every_action_can_be_signed() -> None:
    ...
    for action in Action:
        token = sign(SECRET, action, 7, now=NOW)
...
def test_a_token_signed_with_another_secret_is_refused() -> None:
    ...
    token = sign("someone-elses-secret", Action.DISMISS, 42, now=NOW)
...
def test_an_edited_job_id_is_refused() -> None:
    ...
    honest = sign(SECRET, Action.APPLIED, 42, now=NOW)
    payload, _, signature = honest.partition(".")
    forged = sign(SECRET, Action.APPLIED, 99, now=NOW).partition(".")[0]
    with pytest.raises(TokenError):
...
def test_a_token_is_still_valid_the_day_before_it_expires() -> None:
    ...
    token = sign(SECRET, Action.APPLIED, 42, now=NOW)
    assert verify(SECRET, token, now=NOW + timedelta(days=89)).job_id == 42


def test_a_token_is_url_safe() -> None:
    """A token goes in a path, so it may not contain a character that needs escaping."""
    token = sign(SECRET, Action.DRAFT_COVER_LETTER, 123456, now=NOW)
...
def test_the_link_is_built_from_the_declared_base_url() -> None:
    ...
    url = action_url("http://localhost:8000/", SECRET, Action.DISMISS, 42, now=NOW)
